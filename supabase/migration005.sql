-- =============================================================================
-- Migration: Domain 5 — Billing, Claims & Audit
-- Tables: invoice, payment, insurance_claim, audit_log
-- Depends on: all prior domains
-- Enforces: BR-3, BR-4, REQ-BILL-1/2/3/4, SEC-3
-- Also: attaches fn_audit_log() trigger to patient + appointment_treatment
--       (both owned by other domains — coordination point in workload doc)
-- =============================================================================

-- -----------------------------------------------------------------------------
-- TABLE: invoice
-- 1:1 with a completed appointment (UNIQUE on appointment_id).
-- NO payable_amount column — it's computed in queries (3NF).
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- TABLE: payment
-- Append-only. No UPDATE/DELETE triggers — the API layer is responsible for
-- never exposing those verbs (per workload division notes).
-- -----------------------------------------------------------------------------
...
-- At most one claim per invoice (UNIQUE on invoice_id).
-- approved_amount can be NULL — arrives later when the mock API responds.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- TABLE: audit_log
-- Cross-cutting. staff_id is NOT NULL per SEC-3, which requires the audit
-- trail to capture user ID for every entry — not an optional field. This is
-- safe: the audited tables (patient, appointment_treatment, invoice) are
-- only ever modified through authenticated API calls, so a real staff_id is
-- always available. The application MUST set app.current_staff_id in the
-- same transaction before any DML on an audited table, or the operation
-- will now fail with a NOT NULL violation — a deliberate fail-fast rather
-- than silently recording an unattributed action.
-- details stores a JSONB old/new diff (much more useful than a flat string).
-- -----------------------------------------------------------------------------
...
-- Cross-domain: lives on Domain 4's table, writes Domain 5's table.
-- Creates an EMPTY shell invoice. The API is responsible for filling
-- subtotal_amount in the same transaction by inserting appointment_treatment
-- rows and recomputing.
-- ON CONFLICT DO NOTHING makes the trigger idempotent — re-marking Completed
-- won't duplicate the invoice.
-- -----------------------------------------------------------------------------
...
-- (SEC-3, REQ-BILL-4)
--
-- One function, attached to multiple tables. Reads TG_TABLE_NAME / TG_OP
-- and the row via to_jsonb(). Captures an old/new diff in `details`.
-- Reads the acting staff from the `app.current_staff_id` session variable
-- (set by the API in the same transaction). NULL if unset — for migrations
-- and seed scripts.
-- -----------------------------------------------------------------------------

-- =============================================================================
-- FIX 1: trg_auto_create_invoice_insert
-- Closes the gap where an appointment inserted directly as 'Completed'
-- (e.g. in dummy/seed data) never gets an invoice, because the existing
-- trigger only watches UPDATE.
-- Reuses the existing fn_auto_create_invoice() function — no new function
-- needed, since OLD is NULL on INSERT and the existing IS DISTINCT FROM
-- check already handles that correctly.
-- =============================================================================

CREATE TRIGGER trg_auto_create_invoice_insert
...
-- =============================================================================
-- FIX 2: trg_payment_overpayment_check
-- Prevents the sum of payments on an invoice from ever exceeding its
-- payable amount (subtotal - insurance_deduction - manual_discount).
-- =============================================================================
CREATE TRIGGER trg_payment_overpayment_check
    BEFORE INSERT ON payment
    FOR EACH ROW
    EXECUTE FUNCTION fn_payment_overpayment_check();


CREATE OR REPLACE FUNCTION fn_payment_overpayment_check()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    -- How much this invoice actually owes
    SELECT subtotal_amount - insurance_deduction - manual_discount
    INTO v_payable
    FROM invoice
    WHERE invoice_id = NEW.invoice_id;

    -- How much has already been paid toward it
    SELECT COALESCE(SUM(amount_paid), 0)
    INTO v_already_paid
    FROM payment
    WHERE invoice_id = NEW.invoice_id;

    -- Reject if this new payment would push the total past what's owed
    IF v_already_paid + NEW.amount_paid > v_payable THEN
        RAISE EXCEPTION
            'Payment of % would exceed payable amount % (already paid %)',
            NEW.amount_paid, v_payable, v_already_paid
            USING ERRCODE = 'check_violation';
    END IF;
